...
/**
 * @api {post} /boss/setAvatar  设置某张公开照片为头像
...
exports.setAvatar = function *(next) {
	...
	if(!addr) {
		return (this.body = {
      ret: 0,
      err: Msg(this.session.user.lan, 76)
    })
	}
...
exports.careList = function *(next) {
	...
	if(care.length > 0) {
		for (let i = care.length - 1; i >= 0; i--) {
			let user = yield User.findOne({_id:  care[i], isActive: {$ne: 'no'}},{avatar: 1, oldAvatar: 1,nickname: 1, auditContent: 1, oldName: 1, age: 1, city: 1, vip: 1, selInfo: 1, sex: 1, character: 1, lovePrice: 1, vipLevel: 1}).exec()
			if(user) {
				_care.push(user)
			}
		}
	...
	if(cared.length > 0) {
		for (let i = cared.length - 1; i >= 0; i--) {
			let user = yield User.findOne({_id:  cared[i], isActive: {$ne: 'no'}},{avatar: 1, oldAvatar: 1,nickname: 1, auditContent: 1, oldName: 1, age: 1, city: 1, vip: 1, selInfo: 1, sex: 1, character: 1, lovePrice: 1, vipLevel: 1}).exec()
			if(user) {
				_cared.push(user)
			}
		}
    ...
    for (var i = 0; i < _care.length; i++) {
      if(_care[i].auditContent && _care[i].auditContent.nickname && _care[i].auditContent.nickname !== '1') {
        if(_care[i].oldName) {
          ...
          if(_user.sex == 1) {
            _care[i].nickname = Msg(this.session.user.lan, 83)
          }else if(_user.sex == 2) {
            _care[i].nickname = Msg(this.session.user.lan, 82)
          }
        }
      }
    }
    ...
    for (var i = 0; i < _cared.length; i++) {
      if(_cared[i].auditContent && _cared[i].auditContent.nickname && _cared[i].auditContent.nickname !== '1') {
        if(_cared[i].oldName) {
          ...
          if(_user.sex == 1) {
            _cared[i].nickname = Msg(this.session.user.lan, 83)
          }else if(_user.sex == 2) {
            _cared[i].nickname = Msg(this.session.user.lan, 82)
          }
        }
      }
    }
  ...
  for (let i = 0; i < _care.length; i++) {
    if(onlines.indexOf(_care[i]._id.toString()) > -1) {
      _care[i].online = 'yes'
    }
  }

	for (let i = 0; i < _cared.length; i++) {
    if(onlines.indexOf(_cared[i]._id.toString()) > -1) {
      _cared[i].online = 'yes'
    }
  }
...
exports.care = function *(next) {
  ...
  if (!this.request.body.id) {
	...
	if(userTo.platform && userTo.registration_id) {
		...
		let notice = user.nickname + Msg(this.session.user.lan, 80)
	...
	if(_trace) {
		if(_trace.photoPri && _trace.photoPri.length > 0) {
			if(_trace.photoPri.indexOf(id) > -1) {
				...
				_trace.photoPrie.splice(_trace.photoPrie.indexOf(id), 1)
...
exports.report = function *(next) {
	...
	if(!_user) {
		return (this.body = {
      ret: 0,
      err: Msg(this.session.user.lan, 81)
    })
	}
...
/**
 * @api {post} /boss/soundCare 设置关注提醒
...
/**
 * @api {post} /boss/soundChat 设置消息提醒
...
exports.hateList = function *(next) {
	...
	if(hates.length > 0) {
		for (let i = hates.length - 1; i >= 0; i--) {
			let user = yield User.findOne({_id:  hates[i], isActive: {$ne: 'no'}}).exec()
			if(user) {
				_hates.push(user)
			}	
		}
    ...
    for (var i = 0; i < _hates.length; i++) {
      if(_hates[i].auditContent && _hates[i].auditContent.nickname && _hates[i].auditContent.nickname !== '1') {
        if(_hates[i].oldName) {
          ...
          if(_user.sex == 1) {
            _hates[i].nickname = Msg(this.session.user.lan, 83)
          }else if(_user.sex == 2) {
            _hates[i].nickname = Msg(this.session.user.lan, 82)
          }
        }
      }
    }
  ...
  for (let i = 0; i < _hates.length; i++) {
    if(onlines.indexOf(_hates[i]._id.toString()) > -1) {
      _hates[i].online = 'yes'
    }
  }
...
exports.browseList = function *(next) {
	...
	if(browse.length > 0) {
		for (let i = browse.length - 1; i >= 0; i--) {
			let user = yield User.findOne({_id:  browse[i], isActive: {$ne: 'no'}}).exec()
			if(user){
				_browse.push(user)
			}
	...
	if(browsed.length > 0) {
		for (let i = browsed.length - 1; i >= 0; i--) {
			let user = yield User.findOne({_id:  browsed[i], isActive: {$ne: 'no'}}).exec()
			if(user){
				_browsed.push(user)
			}
    ...
    for (var i = 0; i < _browse.length; i++) {
      if(_browse[i].auditContent && _browse[i].auditContent.nickname && _browse[i].auditContent.nickname !== '1') {
        if(_browse[i].oldName) {
          ...
          if(_user.sex == 1) {
            _browse[i].nickname = Msg(this.session.user.lan, 83)
          }else if(_user.sex == 2) {
            _browse[i].nickname = Msg(this.session.user.lan, 82)
          }
        }
      }
    }
    ...
    for (var i = 0; i < _browsed.length; i++) {
      if(_browsed[i].auditContent && _browsed[i].auditContent.nickname && _browsed[i].auditContent.nickname !== '1') {
        if(_browsed[i].oldName) {
          ...
          if(_user.sex == 1) {
            _browsed[i].nickname = Msg(this.session.user.lan, 83)
          }else if(_user.sex == 2) {
            _browsed[i].nickname = Msg(this.session.user.lan, 82)
          }
        }
      }
    }
  ...
  for (let i = 0; i < _browse.length; i++) {
    if(onlines.indexOf(_browse[i]._id.toString()) > -1) {
      _browse[i].online = 'yes'
    }
  }

  for (let i = 0; i < _browsed.length; i++) {
    if(onlines.indexOf(_browsed[i]._id.toString()) > -1) {
      _browsed[i].online = 'yes'
    }
  }
...
/**
 * @api {post} /boss/delBrowse  用户删除浏览历史
...
/**
 * @api {post} /boss/delBrowsed  用户删除谁看了我
...
exports.ranking = function *(next) {
  ...
  function mathRand() { 
	...
	if(sex == 1) {
		...
		lists = yield User.find({sex: 1, mock: false}).sort({rmbTotal: -1}).limit(6).exec()
	...
	if(sex == 2) {
		...
		for (let i = traces.length - 1; i >= 0; i--) {
			let user = yield User.findOne({_id:  traces[i].userid}).exec()
			if(user) {
				lists.push(user)
			}
		}
